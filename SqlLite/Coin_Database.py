...
def refresh1():
    # messagebox.showinfo("Porfolio Message","Cant Click till 1 min")
    myPortfolio()
...
def nav_itm():
    ...
    app_url="https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?start=1&limit=20&convert=USD&CMC_PRO_API_KEY=66557877-3ca1-4aa0-802a-bbb9a1173d03"
    link_menu.add_command(label='API Hit Page',command=lambda:webbrowser.open(app_url,new=1))
...
def myPortfolio():
    ...
    # api=""
    # refresh=Button(pyc,text="REFRESH",command=reset,bg="grey",fg="Blue",font="Lato 12 bold",padx="5",pady="5",borderwidth=2,relief="groove")
    # refresh.grid(row=coin_row+1,column=7,sticky=N+S+E+W)

    api=""
    refresh=Button(pyc,textvariable='hiii',text="REFRESH",command=refresh1,bg="grey",fg="Blue",font="Lato 12 bold",padx="5",pady="5",borderwidth=2,relief="groove",state='disable')
    refresh.grid(row=coin_row+1,column=7,sticky=N+S+E+W,)
    # widget -w=Button(refresh)
    # if refresh.grab_status()=='disable':
    #     print(refresh.grab_status)
    #     messagebox.showinfo("Porfolio Message","Cant Click till 1 min")

    for item in refresh.keys():
        print(item,":", refresh[item])
